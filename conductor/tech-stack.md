...
- **Frontend:** Vitest + React Testing Library (249 tests as of 2026-08-25, helpers `tauri-driver`/`isolated-dir`/`fixtures`/`tauri`/`prayer`)
  > **Note (2026-08-25):** Track `ux-resilience_20260825` added the component-testing stack (`@testing-library/react` + `@testing-library/jest-dom` + jsdom, registered via `src/test/setup.ts` with `afterEach(cleanup)`; component tests opt in with `// @vitest-environment jsdom`), a shared `QueryError` retry component (`src/components/query-error.tsx`, used across Today, daily card, Log analytics, Calendar and the upcoming-events strip), and a structural EN/ID catalog parity test in `locale.test.ts` (identical key paths + matching `{{placeholder}}` tokens).
